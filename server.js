...
function rewriteHtml(html, baseUrl, serverHost, proxyBase) {
  ...
  var adBlockerScript = '<script>'
    ...
    // ── L14: CSS ad hiders (aggressive — covers interstitials, overlays, banners) ──
    ...
    + '#adfox,'
    + '[class*=\\"overlay\\"][style*\\"z-index\\"],[class*=\\"modal\\"][style*\\"z-index\\"],'
    + '[class*=\\"splash\\"],[class*=\\"interstitial\\"],[class*=\\"preroll\\"],'
    + '[class*=\\"rewarded\\"],[class*=\\"video-ad\\"],[class*=\\"videoad\\"],'
    + '[class*=\\"ad-notice\\"],[class*=\\"ad-wall\\"],[class*=\\"adblock\\"],'
    + '[class*=\\"ad-screen\\"],[class*=\\"adScreen\\"],'
    + '[id*=\\"loko\\"],[class*=\\"loko\\"],'
    + '[class*=\\"gmasdk\\"],[id*=\\"gmasdk\\"]'
    ...
    + 'body>div[style*\\"z-index: 2147483647\\"],body>div[style*\\"z-index:2147483647\\"],'
    + 'body>div[style*\\"position: fixed\\"][style*\\"z-index\\"],'
    + 'body>div[style*\\"position:fixed\\"][style*\\"z-index\\"]'
    + '{display:none!important;visibility:hidden!important;z-index:-1!important;pointer-events:none!important;}'
    ...
    + 'if(/google_ads|googletag\\.cmd|pbjs\\.que|__gads|adServer/i.test(txt))return true;'
    ...
    + 'if(/coinhive|cryptoloot|coinimp|minero\\.cc|WebAssembly.*miner/i.test(txt))return true;'
    + 'if(/fuckAdBlock|blockAdBlock|adBlockDetect/i.test(txt))return true;'
    + 'return false;}'
    + 'function sweep(){'
    + '  var a=document.querySelectorAll("script[src],iframe[src],ins.adsbygoogle,amp-ad,amp-embed,link[rel=\\"stylesheet\\"][href]");'
    ...
    + '  var adDivs=document.querySelectorAll("[id*=\\"sdk__\\"],[class*=\\"sdk__\\"],[id*=\\"google_ads\\"],[class*=\\"adsbygoogle\\"],[id*=\\"imaContainer\\"],[class*=\\"ad-container\\"],[class*=\\"ad-wrapper\\"],[class*=\\"ad-overlay\\"],[class*=\\"interstitial\\"],[class*=\\"preroll\\"],[class*=\\"rewarded\\"],[class*=\\"loko\\"],[class*=\\"gmasdk\\"],[id*=\\"loko\\"],[id*=\\"gmasdk\\"]");'
    + '  for(var i=0;i<adDivs.length;i++){if(adDivs[i].tagName!=="IFRAME")adDivs[i].remove();}'
    + '  var overlays=document.querySelectorAll("div");'
    + '  for(var i=0;i<overlays.length;i++){var el=overlays[i];var s=el.style;'
    + '    if(s.position==="fixed"&&parseInt(s.zIndex)>=9999&&el.offsetWidth>200&&el.offsetHeight>100){el.remove();}'
    + '    if(s.position==="fixed"&&s.top==="0px"&&s.left==="0px"&&s.width==="100%"&&s.height==="100%"&&parseInt(s.zIndex)>=100){el.remove();}}'
    ...
    + 'var obs=new MutationObserver(function(m){for(var i=0;i<m.length;i++){var r=m[i];var nd=r.addedNodes;for(var j=0;j<nd.length;j++){var n=nd[j];if(n.nodeType===1&&(bad(n)||isAdScript(n)))n.remove();}}});'
    ...
    + 'setInterval(function(){sweep();sweepAllShadows();},500);'
    + 'setTimeout(function(){sweep();sweepAllShadows();},200);setTimeout(function(){sweep();sweepAllShadows();},500);setTimeout(function(){sweep();sweepAllShadows();},1000);setTimeout(function(){sweep();sweepAllShadows();},2000);setTimeout(function(){sweep();sweepAllShadows();},3000);setTimeout(function(){sweep();sweepAllShadows();},5000);'
